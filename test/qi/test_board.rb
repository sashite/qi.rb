...
Test("EPIN string pieces on 2D board") do
  sq, pc = Qi::Board.validate([["K^", nil], [nil, "k^"]])
  raise "expected [4, 2], got [#{sq}, #{pc}]" unless sq == 4 && pc == 2
end
...
Test("raises for empty inner array (2D)") do
  Qi::Board.validate([[]])
  raise "should have raised"
rescue ArgumentError => e
  raise "wrong message: #{e.message}" unless e.message == "board must not be empty"
...
Test("raises for multiple empty inner arrays (2D)") do
  Qi::Board.validate([[], []])
  raise "should have raised"
rescue ArgumentError => e
  raise "wrong message: #{e.message}" unless e.message == "board must not be empty"
...
Test("raises for empty inner arrays (3D)") do
  Qi::Board.validate([[[], []]])
  raise "should have raised"
rescue ArgumentError => e
  raise "wrong message: #{e.message}" unless e.message == "board must not be empty"
...
Test("mixed arrays and non-arrays at same level") do
  Qi::Board.validate([["a", "b"], "c"])
  raise "should have raised"
rescue ArgumentError => e
...
Test("array found where flat square expected") do
  Qi::Board.validate(["a", ["b", "c"]])
  raise "should have raised"
rescue ArgumentError => e
...
Test("accepts 255-element 1D board") do
  board = Array.new(255) { nil }
  sq, pc = Qi::Board.validate(board)
  raise "expected [255, 0]" unless sq == 255 && pc == 0
end

Test("rejects 256-element 1D board") do
...
Test("rejects oversized inner dimension") do
  board = [Array.new(256) { nil }]
  Qi::Board.validate(board)
  raise "should have raised"
rescue ArgumentError => e
...
Test("rejects oversized outer dimension (2D board with 256 rows)") do
  board = Array.new(256) { [nil] }
  Qi::Board.validate(board)
  raise "should have raised"
rescue ArgumentError => e
...
Test("mixed types on 2D board") do
  sq, pc = Qi::Board.validate([[:a, nil], [nil, "b"]])
  raise "expected [4, 2]" unless sq == 4 && pc == 2
end

Test("mixed types on 3D board") do
  sq, pc = Qi::Board.validate([[["a", nil], [nil, :b]]])
  raise "expected [4, 2]" unless sq == 4 && pc == 2
end
